...
describe("evidence helpers", () => {
  it("creates, merges, writes, and reloads EA evidence records", () => {
    ...
    const replacement = createEaEvidenceRecord(
      "SVC-auth",
      "test",
      "failed",
      "ci/test",
  ...
  it("validates freshness, failed statuses, and evidence coverage", () => {
    ...
    const issues = validateEaEvidence(
      evidence,
    ...
    expect(issues.some((issue) => issue.rule === "ea:evidence/freshness")).toBe(
      true,
    );
    expect(issues.some((issue) => issue.rule === "ea:evidence/status")).toBe(
      true,
    );
    expect(issues.some((issue) => issue.rule === "ea:evidence/coverage")).toBe(
      true,
    );

    const summary = summarizeEaEvidence(
      evidence,
      [artifact, coverageGapArtifact],
      {
        freshnessWindowDays: 30,
...
describe("evidence CLI", () => {
  ...
  it("fails validation when evidence contains current error records", () => {
    ...
    expect(
      readJsonFile<{ records: Array<{ artifactId: string }> }>(
        dir,
        "ea/evidence/ea-evidence.json",
