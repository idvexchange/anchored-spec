...
function makeException(id: string, expiresAt: string) {
  return makeArtifact({
    id,
    kind: "exception",
    status: "active",
    ...
    scope: { artifactIds: ["SVC-auth"], rules: ["ea:systems/example"] },
  });
}
...
describe("exception reporting", () => {
  it("classifies active, expiring, and expired exceptions", () => {
    const entities = [
      toBackstageEntity(makeException("EXC-active", "2099-01-01")),
      toBackstageEntity(
        makeException(
          "EXC-soon",
      ...
      toBackstageEntity(makeException("EXC-expired", "2020-01-01")),
    ];
    ...
    expect(renderExceptionReportMarkdown(report)).toContain(
      "# Exception Report",
    );
  });
  ...
  it("builds a current report index without legacy report aliases", () => {
    const entities = [
      toBackstageEntity(makeArtifact({ id: "SVC-auth", kind: "service" })),
      toBackstageEntity(makeException("EXC-active", "2099-01-01")),
    ];
...
describe("report CLI", () => {
  it("renders the exceptions view as JSON", () => {
    ...
    writeManifestProject(dir, [makeException("EXC-active", "2099-01-01")]);

    const result = runCli(
      ["report", "--view", "exceptions", "--format", "json"],
      dir,
    );
    expect(result.exitCode).toBe(0);

    const payload = JSON.parse(result.stdout) as { summary: { total: number } };
  ...
  it("writes the current --all report set plus the report index", () => {
    ...
    writeManifestProject(dir, [
      ...
      makeException("EXC-active", "2099-01-01"),
    ]);

    const result = runCli(
      ["report", "--all", "--format", "json", "--output-dir", "reports"],
      dir,
    );
    expect(result.exitCode).toBe(0);
    ...
    const index = readJsonFile<{ reports: Array<{ name: string }> }>(
      dir,
