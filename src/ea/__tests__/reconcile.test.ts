...
describe("reconcileEaProject", () => {
    ...
    it("includes the trace step for entity traceRefs and can skip it explicitly", async () => {
        ...
        expect(withTrace.traceReport?.brokenTraceRefs[0]?.path).toBe("docs/missing.md");
        expect(renderReconcileOutput(withTrace)).toContain('traceRef "docs/missing.md"');
    ...
    it("fails validation when entity loading reports parse errors", async () => {
        ...
        writeTextFile(dir, ".anchored-spec/config.json", JSON.stringify({
            schemaVersion: "1.0",
            entityMode: "manifest",
            manifestPath: "catalog-info.yaml"
        }, null, 2) + "\n");
        writeTextFile(dir, "catalog-info.yaml", [
            "---",
        ...
        expect(report.validationResult?.errors.some((error) => error.rule === "ea:backstage:parse-error")).toBe(true);
    });
});
