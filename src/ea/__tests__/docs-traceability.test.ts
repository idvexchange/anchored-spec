...
describe("Document Traceability", () => {
    ...
    describe("scanDocs", () => {
        let tmpDir: string;
        beforeEach(() => {
            tmpDir = join(tmpdir(), `ea-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    describe("buildDocIndex", () => {
        it("builds inverted index from scanned docs", () => {
            ...
            expect(index.get("component:web")).toHaveLength(1);
            expect(index.get("component:web")![0].relativePath).toBe("docs/a.md");
            expect(index.get("component:api")).toHaveLength(2);
            expect(index.get("component:api")!.map((d) => d.relativePath)).toEqual([
                "docs/a.md",
                "docs/b.md",
            ]);
        ...
        it("handles docs with no artifact IDs", () => {
            ...
            expect(index.has("component:web")).toBe(true);
        });
    });
    // ─── discoverFromDocs ───────────────────────────────────────────
    describe("discoverFromDocs", () => {
        ...
        it("scaffolds drafts for missing artifacts", () => {
            ...
            expect(result.drafts[0]!.suggestedId).toBe("component:auth-core");
            expect(result.drafts[0]!.kind).toBe("Component");
            expect(result.drafts[0]!.schema).toBe("service");
            expect(result.drafts[1]!.suggestedId).toBe("api:auth-v1");
            expect(result.drafts[1]!.kind).toBe("API");
            expect(result.drafts[1]!.schema).toBe("api-contract");
        ...
        it("reports invalid refs", () => {
            const docs = [makeDoc("docs/misc.md", ["UNKNOWN-thing", "component:real"])];
            const existing = [];
            const result = discoverFromDocs(docs, existing);
            expect(result.drafts).toHaveLength(1);
            expect(result.drafts[0]!.suggestedId).toBe("component:real");
            expect(result.invalidRefs).toEqual(["UNKNOWN-thing"]);
        });
        ...
        it("preserves the exact artifact ID from frontmatter", () => {
            ...
            expect(result.drafts[0]!.kind).toBe("API");
            expect(result.drafts[0]!.schema).toBe("api-contract");
        ...
        it("handles IDs without valid prefix format", () => {
            ...
            expect(result.invalidRefs).toEqual(["lowercase-thing", "nohyphen"]);
        });
    });
});
