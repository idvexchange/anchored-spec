...
describe("CLI v2 commands", () => {
  ...
  it("creates a Backstage entity and exposes it through top-level status", () => {
    ...
    expect(status.byKind.application).toBe(1);
  ...
  it("treats removal of an active entity as breaking in diff --compat", () => {
    ...
    writeManifestProject(dir, [
      makeArtifact({ id: "SVC-auth", kind: "service", status: "active" }),
    ]);
    ...
    expect(
      spawnSync("git", ["config", "user.name", "Test User"], { cwd: dir, encoding: "utf-8" }).status,
    ).toBe(0);
    expect(
      spawnSync("git", ["config", "user.email", "test@example.com"], { cwd: dir, encoding: "utf-8" }).status,
    ).toBe(0);
    expect(
      spawnSync("git", ["add", "."], { cwd: dir, encoding: "utf-8" }).status,
    ).toBe(0);
    expect(
      spawnSync("git", ["commit", "-m", "base"], { cwd: dir, encoding: "utf-8" }).status,
    ).toBe(0);
